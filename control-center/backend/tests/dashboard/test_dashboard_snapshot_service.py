...
def test_snapshot_service_transports_rtsp_health_to_active_connections() -> None:
    """Debe transportar RTSP health hasta CONNECTED CLIENTS."""
    ...
    assert result.active_connections is not None
    assert result.active_connections.connection_count == 1
    assert result.active_connections.connections[0].health == "HEALTHY"
    assert (
...
def test_snapshot_service_transports_hls_health_to_active_connections() -> None:
    """Debe transportar HLS health hasta CONNECTED CLIENTS."""
    ...
    assert result.active_connections is not None
    assert result.active_connections.connection_count == 1
    assert result.active_connections.connections[0].protocol == "HLS"
    assert result.active_connections.connections[0].health == "HEALTHY"
    assert (
        result.active_connections.connections[0].bitrate_bps
        == 6_000_000
