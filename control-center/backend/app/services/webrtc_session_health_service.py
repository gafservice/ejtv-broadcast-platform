"""Servicio temporal especializado de salud WebRTC."""
...
class WebRTCSessionHealthService:
    """Construye salud WebRTC a partir de observaciones temporales."""
    ...
    ) -> tuple[WebRTCSessionHealth, ...]:
        """Construye salud para las sesiones WebRTC observadas."""

        sessions: list[WebRTCSessionHealth] = []

        for session in current_snapshot.sessions:
            if session.protocol is not SessionProtocol.WEBRTC:
                continue
            ...
            message = (
                "Insufficient temporal evidence for WebRTC session."
            )
            ...
            if previous_snapshot is not None:
                ...
                if (
                    ...
                    and previous_session.protocol
                    is SessionProtocol.WEBRTC
                    ...
                    if (
                        ...
                        message = (
                            "WebRTC publisher has observed effective traffic."
                            if session.role is SessionRole.PUBLISHER
                            else
                            "WebRTC reader has observed effective traffic."
                        )
